...
class BillController extends Controller {
  async add() {
    ...
    try {
      ...
      // 可以理解为，我登录A账户，那么所做的操作都得加上A账户的id，后续获取的时候，就过滤出 A 账户 id 的账单信息.
  ...
  async list() {
    const { ctx, app } = this;
    // 获取， 日期 date， 分页数据， 类型 type_id，这是都是前端传过来的数据
    ...
    try {
      ...
      let listMap = _list.reduce((curr, item) => {
        ...
        // 如果能在累加的数组中找到当前项日期 date，那么在数组中的加入当前项到bills数组
        if (curr && curr.length && curr.findIndex(item => item.date == date) > -1) {
          const index = curr.findIndex(item => item.date == date);
          curr[index].bills.push(item);
        }
        // 如果在累加的数组中找不到当前日期，那么再新建一项
        if (curr && curr.length && curr.findIndex(item => item.date == date) == -1) {
          curr.push({
            date,
            bills: [item],
        ...
        // 如果curr为空数组，则默认添加第一个账单项 item，格式化为下列模式
        if (!curr.length) {
          curr.push({
            date,
            bills: [item],
  ...
  async data() {
    ...
    try {
      ...
      const _data = result.filter(item =>
        (Number(item.date) > start && Number(item.date) < end)
      )
      // 总支出
